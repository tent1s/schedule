...
class NewTimeRowViewModelFactory(private val dataSource: TimetableDatabaseDao,
                              private val application: Application, private val timetableId : String, private val weekId : Int, private val firebase: DatabaseReference) : ViewModelProvider.Factory {
    @Suppress("unchecked_cast")
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(NewTimeRowViewModel::class.java)) {
            return NewTimeRowViewModel(dataSource, application, timetableId, weekId, firebase) as T
        }
